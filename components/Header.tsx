...
const Header = () => {
  ...
  return (
    <header className="bg-theme1/70 fixed right-0 left-0 z-1000 flex max-w-screen flex-row items-center justify-between px-6 py-3 sm:px-10 lg:px-18">
      <div>
        <p className="font-orbitron text-4xl font-semibold text-slate-100 uppercase">
          ŠK
        </p>
      </div>
      ...
      {/* ------------------mobile navigation---------------------------- */}
      <div className="ml-auto block rounded-2xl p-2 lg:hidden">
        <div
      ...
      <nav
        className={cn(
          "border-theme3 bg-theme4 fixed top-0 bottom-0 left-0 z-9999 w-[300px] border-r-2 bg-[url(/space_bacg.jpg)] bg-cover bg-no-repeat shadow-[inset_-50px_0_50px_-50px_var(--theme3)] transition-all duration-300 max-[410px]:w-[260px] sm:w-[330px] lg:hidden",
          hambActive ? "translate-x-0" : "-translate-x-full",
        )}
      >
        <div className="from-theme1/50 to-theme2/80 absolute inset-0 -z-10 bg-linear-to-r" />
        <div className="scrollbar-slate flex h-full max-h-screen flex-col justify-between gap-25 overflow-y-auto py-12">
          <ul className="flex flex-col gap-6 text-2xl">
            <li className="w-full">
              <Link
                href="#"
                className="font-orbitron relative block w-full cursor-pointer px-3 py-1 text-2xl font-bold tracking-widest text-slate-100 capitalize"
              >
                Home
              </Link>
            </li>
